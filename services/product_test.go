...
func Test_IProductService_Find(t *testing.T) {
	...
	assert.Nil(t, err, "No error")
	assert.Equal(t, int64(1), prod.ID, "Product ID is 1")

	//---
	t.Log("2. Expect error: id must be >= 1")
	prod, err = serv.Find(0) //assume db serial starts with 1
	assert.Error(t, err, "ID parameter must exists")
	...
	assert.Error(t, err, "Error product not found")
...
func Test_IProductService_Create(t *testing.T) {
	...
	assert.Nil(t, err, "No error")
	assert.Equal(t, "Product 1", prod.Name, "Product name is Product 1")
	assert.NotEqual(t, 0, prod.CreateTs, "Product created timestamp is filled")
	...
	prod, err = serv.Create([]byte(jsonReq))
	assert.Error(t, err, "Product price must be > 0")
	...
	prod, err = serv.Create([]byte(jsonReq))
	assert.Error(t, err, "Product category must be filled")
	assert.Equal(t, 400, err.Code)
	...
	prod, err = serv.Create([]byte(jsonReq))
	assert.Error(t, err, "Product name must be filled")
	assert.Equal(t, 400, err.Code)
	...
	assert.Error(t, err, "Product name must be filled")
	assert.Equal(t, 400, err.Code)
	assert.Equal(t, "Invalid JSON request", err.Message)
